...
export class Game {
   ...
   * @param {numbe r} values.d2 - Valor del segundo dado.
   ...
   */
  async endGameManual() {
    // 1) Calcular standings (usa tu Rules actual)
    const standings = this.rules.computeStandings(this);

    // 2) Mostrar pop-up con posiciones finales
    this.ui?.modals?.showStandings?.(standings);

    // 3) Enviar TODOS los jugadores al ranking
    //    El backend espera: { nick_name, score, country_code }
    for (const s of standings) {
      try {
        await this.api.scoreRecorder({
          nick_name: s.nick,
          score: s.score,
          country_code: s.country || "US",
        });
      } catch (e) {
        this.ui?.notifier?.warn?.(
          `No se pudo registrar a ${s.nick}`,
          "Ranking"
        );
      }
    }

    // 4) Feedback y refresco opcional
    this.ui?.notifier?.success?.(
      "Puntajes enviados al ranking.",
      "Juego terminado"
    );
    this.ui?.refresh?.();
  }
